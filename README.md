...
## How it was built
	...
	- my original can be found at https://codepen.io/TurboTodd2k/pen/RNGevLK, it's a very basic implimentation but if you want something simple to mess around with it's not a bad starting point. If it seems like it's just one of the examples from the viz.js site with the node data swapped out you'd be correct
...
## To do
	- make adjustment to ranges on the collab result lookups
	- more fiddling with how the graph is laying out
	- fix html so the correct button is selected on page load
	- make sure there's enough variety in color for groups and that the legend is up to date
