...
## Features
Has a Google sheet based backend, data can be edited without having to fiddle with code so many folks can contribute
...
The drop downs to select collab partners are pretty clunky and run off of a look up to columns on collab support 1 & collab support 2 in the spreadsheet. When you insert a row to add a person you have to remember to create a lookup for them on collab support 2 and then use that as data validation for the collab dropdown

In general if you do a bunch of edits to the data it's best to export all of the sheets as it's easy to forget what you've edited. If you only export part of the data and any of the id's for talent have changed you get an error when you try to load the page
